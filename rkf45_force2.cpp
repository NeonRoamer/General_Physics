...
std::string data_file = "force_results.txt";
...
int main()
{
  ...
  std::uniform_real_distribution rand_acc(1e15, 1e17);    //Set bounds here
  ...
  while(n_sims<max_sims)
  {
    ...
    double asteroid_force_mag{rand_acc(mt)}; //N 
    ...
    std::vector<std::vector<double>> const_acc{ std::vector<double>(3), std::vector<double>(3), asteroid_force_vec / mass_ceres };
    ...
    if(earth_hit) oss << 1 << ", " << asteroid_force_mag << ", " << imparted_theta << '\n';
    else if(time<time_max)
    {
      std::cout << "Simulation stuck, last error = " << error << std::endl;
      continue;
    }
    else oss << 0 << ", " << asteroid_force_mag << ", " << imparted_theta << '\n';
