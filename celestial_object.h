...
private:
  ...
  vector<double> m_old_acceleration{0}; // Units in m / s^2
  vector<double> m_new_acceleration{0}; // Units in m / s^2

public:
  vector<double> coordinate{vector<double>(3, 0)}; // Units in m
  vector<double> velocity{vector<double>(3, 0)}; // Units in m/s
  ...
  double get_distance(Celestial_object& object);
  ...
  void reset_acceleration(){m_new_acceleration = vector<double>(3, 0);}
