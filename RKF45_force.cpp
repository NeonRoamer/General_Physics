//Testing magnitude and direction of constant force
//Sets moon orbit in xy plane, and rotates the incoming asteroid by a random inclination angle
//Constant force is delivered at a random theta and phi relative to asteroid
//Constant force does not change orientation in lab frame
#include<iostream>
#include<vector>
#include<string>
#include<cmath>
...
const std::string data_file = "force_results.txt";
...
const double max_force = 1.5e16;  //N
const double min_force = 1e15;  //N
...
int main()
{
  ...
  std::vector<double> asteroid_position = std::vector<double>{ init_r, 0, 0 };
  std::vector<double> asteroid_velocity = std::vector<double>{ -init_v, 0, 0 };
  bodies.emplace_back(mass_ceres, radius_ceres, asteroid_position, asteroid_velocity);
  std::vector<std::vector<double>> const_acc(3, std::vector<double>(3));
  ...
  std::uniform_real_distribution rand_force(min_force, max_force); // May change this to logarithmic
  double asteroid_force_mag;
  std::vector<double> asteroid_force_vec(3);
  ...
  oss << "Outcome, Inclination, Force mag, Force theta, Force phi\n";

  while(n_sims < max_sims)
  {
    std::cout<<n_sims<<std::endl;
    //Reset model
    inc_angle = rand_angle(mt) / 4;
    imparted_theta = rand_angle(mt) / 2;
    imparted_phi = rand_angle(mt);

    asteroid_force_vec = std::vector<double>{ cos(imparted_theta), sin(imparted_theta) * cos(imparted_phi), sin(imparted_theta) * sin(imparted_phi) };
    asteroid_force_mag = rand_force(mt);
    asteroid_force_vec *= asteroid_force_mag;
    asteroid_position = std::vector<double>{ init_r, 0, 0 };
    asteroid_velocity = std::vector<double>{ -init_v, 0, 0 };
    ...
    const_acc[3] = asteroid_force_vec;
    ...
    while(time < time_max)
    {
      ...
      do
      {
        ...
        n_trials++;
        if(n_trials >= max_trials || earth_hit) break;
    ...
    else if(earth_hit) oss << 1 << '\t' << std::round(inc_angle) << '\t' << std::round(asteroid_force_mag) << '\t' << std::round(imparted_theta) << '\t' << std::round(imparted_phi) << '\n';
    else oss << 0 << '\t' << std::round(inc_angle) << '\t' << std::round(asteroid_force_mag) << '\t' << std::round(imparted_theta) << '\t' << std::round(imparted_phi) << '\n';
